...
function loadTopFavorites(topFaves) {
  for (const spell of topFaves) {
    const url = urlBase + spell.name;
    loadJsonFetch(url);
  }
}
...
async function loadJsonFetch(url, type) {
  ...
  if (type == 0) {
    const newCard = spellResult.createResult(json.results[0]);
